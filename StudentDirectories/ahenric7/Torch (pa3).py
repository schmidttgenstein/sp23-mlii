    ...
    def __init__(self,params:list=None,dims:list=None, epoch:int= 250, lr = 0.05,):
        ...
        for j in range(len(dims)-1):
            ...
            if params is not None:
                ...
                od[f"linear_{j}"].bias = nn.Parameter(torch.tensor(bias[j],requires_grad = True).float())# possibly also needt to be made a float
...
class FCNetFS(MLPipeline):
    def __init__(self,params:list=None, dims:list=None,epochs:int = 1500,lr = .01,):
        ...
        if params is None:
            ...
            for j in range(int(len(dims)-1)):
                ...
                weights.append(w)
                bias.append(b.reshape(dims[j+1], 1))
        else:
            weights = params[0]
            bias = params[1]
    ...
    def full_forward(self,x):
        ...
        for weight in self.weights:
            ...
            z_pre = weight @  a # noted in the original, the weight being transformed does fix that part of the issue. I'm going to try and go back and fix this stuff at the place the weights are made.
            print("XXX", z_pre.shape)
            z = z_pre + self.bias[j].reshape(z_pre.shape[0],1)
            if j < self.n_layers - 1: # checks if there is still a layer left to calculate
                a = self.activation(z)
            else: 
                a = z
            layer_acts.append(a)
...
if __name__ == "__main__":
    #Step 0: No errors here, instantiate a network and "look at it" (you can skip)
    ...
    #print(bnet.forward_stack)
    #Before continuing, examine your bnet.forward_stack to ensure that layers are coordinated as you expected
    n_samp = 350
    tspace = np.linspace(-15,15,n_samp).reshape(1,n_samp)
    y1 =  anet.forward(tspace)
    print(torch.tensor(tspace).float())
    print(torch.from_numpy(tspace))
    y2 = bnet.forward(torch.tensor(tspace).float()) # appears that I may need to transform this into a tensor instead of a ndarray.
    #Step 1.5 sanity check: can you reproduce From Scratch network with prespecified weights params?
    a2net = FCNetFS(params=in_weights) 
