...
class Perceptron:
    ...
    def get_wrongs(self,x_data,y_data):
        ...
        wrongs_idx = [i for i in range(x_data.shape[1]) if np.sign(self.w @ x_data[:, i]) != y_data[i]]
        x_wrongs = x_data[:, wrongs_idx]
        y_wrongs = y_data[wrongs_idx]
        size = y_wrongs.shape[0]
        return (size, x_wrongs, y_wrongs)

    def fit_w(self,x_data,y_data,printing = False):
        #This method should implement the perceptron Algorithm
        size, x_wrongs, y_wrongs = self.get_wrongs(x_data,y_data)
        iters = 0
        while size > 0:
            self.w = self.w + y_wrongs[-1] * x_wrongs[:, -1]
            iters += 1
                #print(self.w)
            if printing:
                print(self.eval_w(x_data, y_data))
            size, x_wrongs, y_wrongs = self.get_wrongs(x_data,y_data)
        return iters, 0

    def eval_w(self,x_test,y_test):
        y_pred = np.array([np.sign(self.w @ x_test[:, i]) for i in range(x_test.shape[1])])
