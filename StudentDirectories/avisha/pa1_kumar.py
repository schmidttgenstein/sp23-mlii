...
class Perceptron:
    ...
    def get_wrongs(self,x_data,y_data):
        ...
        return len(x_wrong), x_wrong, y_wrong """
        wrong = (y_data * (self.w @ x_data)) < 0
        num_wrong = sum(wrong)
        return num_wrong, x_data[:,wrong], y_data[wrong]
    ...
    def fit_w(self,x_data,y_data,printing = False):
        ...
        n_steps = 0
        while size_wrong > 0:
            idx = np.random.randint(size_wrong)
            n_steps += 1 
            update = self.w + x_wrong[:,idx]*y_wrong[idx]
            self.w = update
            size_wrong,x_wrong,y_wrong = self.get_wrongs(x_data,y_data)

            if printing: 
                print(f'iteration {n_steps}, number of wrongs {size_wrong}')


        upper_bound = (la.norm(self.w) * la.norm(x_data,axis = 0).max())**2
            ### Algo goes here
        ## update w and return # of steps to completion 
        return n_steps, upper_bound

    def eval_w(self,x_test,y_test):
        #evaluate accuracy of perceptron on data set x_test, y_test
        size_wrong,x_wrong,y_wrong = self.get_wrongs(x_test,y_test)
        accuracy = size_wrong/len(y_test)
