...
class FCNetFS(MLPipeline):
    ...
    def backward(self,y_layers,y_truth):
        ...
        '''
        y_sc = y_layers[-1] # grab the output of network
        _,dc = self.cost(y_sc,y_truth) #start with gradient of cost wrt output (should be 2 x m) 
        ...
        for j in range(len(y_layers)-1):
            ...
            #print(a_prev)
            dadz = y_layer*(1-y_layer) # (I'm not enterely sure why y_layer correspond to the sigmoid activation)
            ...
            dw = (1/a_prev.shape[1])*(a_prev @ dc.transpose()).transpose() # make sure you get the dimensions aligned. There may be some 
...
if __name__ == "__main__":
    ...
    xtr,ytr,xte,yte = dg.gen_data(mu_factor = 10)
    ...
    #c is cross-entropy, dc is gradient of the cost
    ...
    plt.show()
    bnet = FCNetFS(dims = [input_dim,20,15,10,2],epochs = 700,lr = 0.75)
